...
<html lang="en">
    ...
    <body>
        <div class="wrapper">
            <h1>YAML Converter</h1>
            <p>
               This tool extracts any text from an HTML template and replaces them with placeholders.
               It also generates a YAML file with all the extracted text blocks.
            </p>
            <?php
            ...
            class HTMLTextExtractor {
               ...
               private $placeholder = '[textblock id="%s"]';
               private $keyMaxLength = 30;
               ...
               private function generateKey($text) {
                  $tokens = explode(' ', trim($text), 10);
                  $key = $tokens[0] ?? '_';
                  for ($i = 1; $i < count($tokens) && strlen($key) < $this->keyMaxLength; ++$i) {
                     $key .= '_' . $tokens[$i];
                  }
                  $key = strtolower($key);
                  $key = preg_replace('#\s+#m', '_', $key);
                  $key = preg_replace('#[^a-z0-9_-]#', '', $key);
                  $key = preg_replace('#^[_-]+#', '', $key);
                  $key = empty($key) ? 'textblock' : $key;
                  $ext = '';

                  for ($i = 1; array_key_exists($key . $ext, $this->texts); ++$i) {
                     $ext = "_$i";
                  }
                  return "$key$ext";
               }

               private function generatePlaceholder(string $key): string {
                  return sprintf($this->placeholder, $key);
               }
               ...
               private function convertTextNode(DOMNode $node) {
                  ...
                  if (!empty($text)) {
                     ...
                     $this->texts[$key] = utf8_decode(preg_replace('#\s+#m', ' ', $text));
                     $node->textContent = $this->generatePlaceholder($key);
                  }
               }
            ...
            if (array_key_exists('html', $_POST)) {
               ...
               if (!$converter->convert($_POST['html'])) {
                  ...
                  $yamlstring = yaml_emit(['text' => $texts], YAML_UTF8_ENCODING, YAML_LN_BREAK);
                  if (!$yamlstring) {
                     $error = 'Something went wrong. Could not generate YAML file.';
                     echo '<pre>'; var_dump($texts); echo '</pre>';
                  }
            ...
            <div class="side-by-side">
               <div>
                  <h2>YAML:</h2>
                  <textarea class="full-width tall"><?= $yamlstring ?></textarea>
               </div>
               <div>
